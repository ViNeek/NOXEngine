...
std::vector<glm::vec3> Utils::Shape::generateSphereMeshScaledAt(int longitude_steps, int latitude_steps, float scale, glm::vec3 position)
{
	// create the sphere
	std::vector<glm::vec3> sphere_data;
	const float PI = 3.141592f;
	...
	if (longitude_steps < 2) longitude_steps = 2;
	if (latitude_steps < 4) latitude_steps = 4;
	float sphere_radius = 1.0f / scale;
	float phi_step = 2 * PI / float(latitude_steps);
	float theta_step = PI / float(longitude_steps);
	float phi = 0;
	float theta = 0;
